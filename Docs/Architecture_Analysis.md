...
### 问题陈述
Core.Systems模块包含990个文件（占代码库24.7%），严重违反单一职责原则，包含三类不同职责：数据定义、管理器、工具类。

### 决策
拆分Core.Systems为三个子模块：
- **Core.Data** (~300文件) - 数据结构、ScriptableObject、配置类
- **Core.Management** (~400文件) - 管理器、控制器、系统服务
- **Core.Utilities** (~290文件) - 扩展方法、工具类、辅助函数

### 理由
1. **单一职责** - 每个模块职责明确
2. **可维护性** - 更小的模块更易理解和维护
3. **依赖清晰** - 明确的依赖关系（Management依赖Data，Utilities独立）
...
### 实施计划
Phase 2执行，建议使用自动化工具批量更新using语句。
...
### 问题陈述
Dungeonator是419个文件的依赖基础，是否需要完整移植？

### 决策
**完整移植Dungeonator**，保持命名空间和API不变。

### 理由
1. **不可替代** - 高度定制化的程序生成系统，无现成替代
2. **核心基础** - 419个依赖者，整个游戏的空间框架
3. **独立命名空间** - 边界清晰，适合整体迁移
4. **替代成本高** - 重写需要数月，风险极高

### 影响范围
Phase 1最高优先级，必须在其他模块之前完成。

### 替代方案
- 方案B：创建适配器层 - 增加复杂度，不推荐
- 方案C：替换为现成库 - 无合适方案
...
### 问题陈述
Brave.BulletScript DSL是268个文件的依赖，Boss战斗的核心，是否保留？

### 决策
**完整保留并移植BulletScript DSL**。

### 理由
1. **不可替换** - 专为ETG设计的弹幕DSL
2. **Boss核心** - 所有Boss弹幕模式依赖此系统
3. **重写成本** - 需要重新设计所有Boss战斗
4. **表达力强** - DSL比代码更适合定义弹幕模式

### 影响范围
Phase 1迁移，与Dungeonator同等优先级。
...
### 问题陈述
FullInspector有343个依赖者，用于序列化，是否保留？

### 决策
**Phase 4替换FullInspector为标准JSON序列化**（JsonUtility或Newtonsoft.Json）。
...
### 影响范围
343个文件需修改序列化代码，主要是Core.Systems（225文件）。

### 实施计划
1. 先迁移Core.Systems
2. 替换FullInspector序列化为JSON
3. 测试数据兼容性
...
### 问题陈述
SpeculativeRigidbody包装Unity Rigidbody2D，提供像素级碰撞，如何处理？

### 决策
**完整移植SpeculativeRigidbody**，保持行为一致性。

### 理由
1. **游戏手感核心** - ETG的精确碰撞是核心体验
2. **性能关键** - 每帧100+角色的物理更新
3. **行为依赖** - 大量游戏逻辑依赖其特定行为

### 影响范围
Phase 1迁移，性能测试必须确保无退化。
...
### 问题陈述
3个循环依赖需要解决：
1. Dungeonator ↔ PlayMaker (1↔12文件)
2. Dungeonator ↔ Pathfinding (2↔2文件)
3. Dungeonator ↔ tk2dRuntime (1↔3文件)

### 决策
1. **移除PlayMaker** - 使用少，Phase 4移除
2. **保持Pathfinding耦合** - 合理依赖，或引入接口抽象
3. **替换tk2dRuntime** - Phase 4替换为Unity 2D

### 理由
仅3个循环依赖（3.5%），都有明确解决方案。
...
### Unity版本
- 当前：Unity 5.x/2017.x（反编译推测）
- 建议：Unity 2021 LTS或更新

### 核心依赖
| 依赖 | 用途 | 状态 | 建议 |
|-----|------|------|------|
| Dungeonator | 程序生成 | 保留 | 完整移植 |
| Brave.BulletScript | 弹幕DSL | 保留 | 完整移植 |
| FullInspector | 序列化 | 替换 | → JSON |
| InControl | 输入 | 替换 | → New Input System |
| tk2dRuntime | 2D工具 | 替换 | → Unity 2D |
| PlayMaker | 可视化脚本 | 移除 | - |
| DaikonForge | UI | 替换 | → Unity UI |
...
1. **Core.Systems → Brave.BulletScript (224文件)** - 弹幕脚本定义
2. **Core.Systems → FullInspector (225文件)** - 序列化
3. **Core.Systems → Dungeonator (114文件)** - 空间查询
4. **Core.Dungeon → Dungeonator (82文件)** - 房间逻辑
5. **Core.Actors → Core.Core (81文件)** - 基类继承

### 依赖路径分析

**路径1影响：** BulletScript必须Phase 1完成
**路径2影响：** FullInspector替换影响最大
**路径3-4影响：** Dungeonator是瓶颈，最先迁移
**路径5影响：** Core.Core基类变更级联影响大

## 5.2 循环依赖详解

已在ADR-006中详细说明，3个循环依赖都有解决方案。

## 5.3 依赖强度分类

| 强度 | 定义 | 模块 | 策略 |
|-----|------|------|------|
| **Strong** | >100文件 | Dungeonator, BulletScript, FullInspector, Core.Core | 不可替换或Phase 4替换 |
| **Medium** | 30-100文件 | Core.Systems依赖, 领域层依赖 | Phase 2-3迁移 |
| **Weak** | <30文件 | 大部分外部库 | Phase 4评估 |
...
- 保持API 100%兼容
- 性能测试确保无退化
- GameActor.Update(), SpeculativeRigidbody性能基准
...
**包含模块：**
1. Core.Systems (990文件，先拆分)
2. Core.Items (175文件)
3. Core.Dungeon (125文件)
4. Core.Actors (167文件)
5. Core.Combat (172文件) - 最后，高风险
...
**包含任务：**
1. FullInspector → JSON (343文件受影响)
2. InControl → New Input System (22文件)
3. tk2dRuntime → Unity 2D (25文件)
4. PlayMaker移除 (14文件)
5. DaikonForge → Unity UI (6文件)
...
```mermaid
graph TD
    Start[开始迁移] --> Phase1{Phase 1<br/>基础层}
    Phase1 -->|完成| Validate1[验证可玩性]
    Validate1 -->|通过| Phase2{Phase 2<br/>核心+领域层}
    Validate1 -->|失败| Fix1[修复问题]
    Fix1 --> Validate1

    Phase2 -->|完成| Validate2[完整测试]
    Validate2 -->|通过| Phase3{Phase 3<br/>表现层}
    Validate2 -->|失败| Fix2[修复问题]
    Fix2 --> Validate2

    Phase3 -->|完成| Validate3[UI/音频测试]
    Validate3 -->|通过| Phase4{Phase 4<br/>外部依赖替换}
    Validate3 -->|失败| Fix3[修复问题]
    Fix3 --> Validate3

    Phase4 -->|完成| Final[最终验收]
    Final -->|通过| Done[迁移完成]
    Final -->|失败| Fix4[修复问题]
    Fix4 --> Final
```
...
| 步骤 | 模块 | 文件数 | 依赖 | 风险 | 并行机会 |
|-----|------|-------|------|------|---------|
| 1 | Core.Core | 377 | 无 | 极高 | 无 |
| 2 | Dungeonator | 67 | Core.Core | 极高 | 无 |
| 3 | Brave.BulletScript | 14 | Core.Core | 高 | 可与Dungeonator并行 |
| 4 | Core.Systems拆分 | 990 | Phase 1 | 高 | 无 |
| 5 | Core.Items | 175 | Core.Systems | 中 | 可并行 |
| 6 | Core.Dungeon | 125 | Dungeonator | 中 | 可并行 |
| 7 | Core.Actors | 167 | Core.Core | 高 | 无 |
| 8 | Core.Combat | 172 | 所有 | 极高 | 无 |
| 9 | Core.UI | 100 | Phase 2 | 低 | 可并行 |
| 10 | Core.Audio | 150 | Phase 2 | 低 | 可并行 |
| 11 | Core.VFX | 67 | Phase 2 | 低 | 可并行 |
| 12 | 外部依赖替换 | 706 | 全部 | 中 | 逐个替换 |
...
| 级别 | 定义 | 依赖者数 | 文件数 |
|-----|------|---------|-------|
| **极高** | 核心基础，大量依赖者，性能敏感 | >200 | >100 |
| **高** | 重要模块，中等依赖 | 50-200 | 50-100 |
| **中** | 常规模块，少量依赖 | 10-50 | 10-50 |
| **低** | 独立模块，极少依赖 | <10 | <10 |

## 7.2 模块风险矩阵

| 模块 | 风险级别 | 依赖者数 | 主要风险 | 缓解策略 |
|-----|---------|---------|---------|---------|
| Core.Core | 极高 | 265 | 基类变更级联影响 | API 100%兼容，充分测试 |
| Core.Systems | 极高 | 224 | 模块过大，拆分风险 | 自动化工具，增量拆分 |
| Dungeonator | 极高 | 419 | 空间系统核心 | 完整移植，行为测试 |
| Brave.BulletScript | 极高 | 268 | Boss战斗核心 | DSL兼容性测试 |
| Core.Combat | 极高 | 72 | 游戏手感，性能热路径 | 性能基准，手感测试 |
| Core.Actors | 高 | 137 | AI复杂度 | 行为树验证 |
| Core.Items | 中 | 35 | 数据驱动，相对安全 | 数据完整性测试 |
| Core.Dungeon | 中 | 7 | 依赖Dungeonator | Phase 1后迁移 |
| Core.UI | 低 | 8 | UI框架混用 | 统一框架 |
| Core.Audio | 低 | 4 | 相对独立 | 音效测试 |
| Core.VFX | 低 | 2 | 最独立 | 视觉验证 |
...
**缓解：**
- 冻结API，不做任何签名变更
- 保持MonoBehaviour字段名不变
- 每个基类变更都需全量回归测试

### 2. Core.Systems (990文件)
**风险：** 拆分时大量using语句需更新
**缓解：**
- 使用Roslyn编写自动化工具
- 分批拆分，每批验证
...
| 路径 | 位置 | 调用频率 | 性能要求 | 优先级 |
|-----|------|---------|---------|--------|
| GameActor.Update() | Core.Core | 100+ actors/frame | <0.1ms/actor | P0 |
| SpeculativeRigidbody.PhysicsUpdate() | Core.Core | 100+ bodies/frame | <0.05ms/body | P0 |
| Projectile.Update() | Core.Combat | 200+ projectiles/frame | <0.05ms/proj | P0 |
| PixelCollider.Query() | Core.Core | 数百次/frame | <0.01ms/query | P0 |
| BulletScript.Execute() | Brave.BulletScript | Boss战斗时高频 | <0.1ms/script | P1 |

## 8.2 性能要求规格

### 目标帧率
- **PC：** 60 FPS稳定（16.67ms/frame）
- **主机：** 60 FPS稳定
- **允许掉帧：** <1%
...
| 系统 | 预算 (ms/frame) | 备注 |
|-----|----------------|------|
| Actor更新 | 3.0ms | 100+ actors |
| 物理+碰撞 | 2.5ms | SpeculativeRigidbody |
| 弹幕更新 | 2.0ms | 200+ projectiles |
| 渲染 | 6.0ms | Unity渲染 |
| 其他 | 3.17ms | UI, 音频等 |
| **总计** | **16.67ms** | 60 FPS |
...
### 基准测试
1. **建立基线** - 迁移前性能数据
2. **每阶段测试** - Phase 1-4后测试
3. **对比分析** - 不能低于基线

### 测试场景
1. **满屏弹幕** - 200+ projectiles
2. **多敌人房间** - 15+ enemies
3. **Boss战斗** - 复杂弹幕模式
4. **长时间运行** - 30分钟无卡顿
...
### FullInspector → JSON迁移

**影响范围：** 343个文件

**迁移步骤：**
1. 为每种数据类型编写JSON序列化器
2. 导出现有FullInspector数据为JSON
3. 验证数据完整性
4. 批量替换序列化代码

**风险：** 数据丢失、格式不兼容

**缓解：** 双写策略（同时写FullInspector和JSON），验证一致性后切换
...
| 数据类型 | 数量 | 重要性 | 验证方法 |
|---------|------|--------|---------|
| 枪械数据 (GunData) | 100+ | 极高 | 对比射速、伤害、弹匣 |
| 物品数据 (ItemData) | 200+ | 极高 | 对比效果描述 |
| 敌人数据 (EnemyData) | 80+ | 高 | 对比血量、AI行为 |
| 弹幕脚本 (BulletScriptSource) | 50+ | 极高 | Boss战斗测试 |
| 房间数据 (RoomData) | 500+ | 高 | 地牢生成测试 |
| 玩家存档 (SaveData) | - | 极高 | 存档兼容性 |
...
**三级验证：**
1. **Level 1：** 自动化脚本验证（数值、字符串）
2. **Level 2：** 单元测试验证（数据加载、序列化）
3. **Level 3：** 集成测试验证（游戏内实际使用）
...
1. **完成Phase 1-2** (3-6个月)
   - Core.Core + Dungeonator + BulletScript
   - Core.Systems拆分
   - 领域层迁移

2. **外部依赖替换评估** (Phase 2期间)
   - FullInspector替换方案验证
   - InControl替换到New Input System
   - tk2dRuntime替换方案
