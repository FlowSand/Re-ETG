...
// Type: AkPropagationPathInfoArray
...
public class AkPropagationPathInfoArray : IDisposable
  {
    ...
    ~AkPropagationPathInfoArray() => this.Dispose();

    public void Reset() => this.m_Count = 0;

    public AkPropagationPathInfoProxy GetPropagationPathInfo(int index)
    {
      return index >= this.m_Count ? (AkPropagationPathInfoProxy) null : new AkPropagationPathInfoProxy(this.GetObjectPtr(index), false);
    }
