// Decompiled with JetBrains decompiler
// Type: dfPivotExtensions
// Assembly: Assembly-CSharp, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: E27C5245-924B-4031-BFBB-14AA632E24E2
// Assembly location: D:\Github\Re-ETG\Managed\Assembly-CSharp.dll

using System;
using UnityEngine;

#nullable disable

namespace ETG.Core.Systems.Utilities
{
    public static class dfPivotExtensions
    {
      public static Vector2 AsOffset(this dfPivotPoint pivot)
      {
        switch (pivot)
        {
          case dfPivotPoint.TopLeft:
            return Vector2.zero;
          case dfPivotPoint.TopCenter:
            return new Vector2(0.5f, 0.0f);
          case dfPivotPoint.TopRight:
            return new Vector2(1f, 0.0f);
          case dfPivotPoint.MiddleLeft:
            return new Vector2(0.0f, 0.5f);
          case dfPivotPoint.MiddleCenter:
            return new Vector2(0.5f, 0.5f);
          case dfPivotPoint.MiddleRight:
            return new Vector2(1f, 0.5f);
          case dfPivotPoint.BottomLeft:
            return new Vector2(0.0f, 1f);
          case dfPivotPoint.BottomCenter:
            return new Vector2(0.5f, 1f);
          case dfPivotPoint.BottomRight:
            return new Vector2(1f, 1f);
          default:
            return Vector2.zero;
        }
      }

      public static Vector3 TransformToCenter(this dfPivotPoint pivot, Vector2 size)
      {
        switch (pivot)
        {
          case dfPivotPoint.TopLeft:
            return (Vector3) new Vector2(0.5f * size.x, (float) (0.5 * -(double) size.y));
          case dfPivotPoint.TopCenter:
            return (Vector3) new Vector2(0.0f, (float) (0.5 * -(double) size.y));
          case dfPivotPoint.TopRight:
            return (Vector3) new Vector2((float) (0.5 * -(double) size.x), (float) (0.5 * -(double) size.y));
          case dfPivotPoint.MiddleLeft:
            return (Vector3) new Vector2(0.5f * size.x, 0.0f);
          case dfPivotPoint.MiddleCenter:
            return (Vector3) new Vector2(0.0f, 0.0f);
          case dfPivotPoint.MiddleRight:
            return (Vector3) new Vector2((float) (0.5 * -(double) size.x), 0.0f);
          case dfPivotPoint.BottomLeft:
            return (Vector3) new Vector2(0.5f * size.x, 0.5f * size.y);
          case dfPivotPoint.BottomCenter:
            return (Vector3) new Vector2(0.0f, 0.5f * size.y);
          case dfPivotPoint.BottomRight:
            return (Vector3) new Vector2((float) (0.5 * -(double) size.x), 0.5f * size.y);
          default:
            throw new Exception($"Unhandled {pivot.GetType().Name} value: {(object) pivot}");
        }
      }

      public static Vector3 UpperLeftToTransform(this dfPivotPoint pivot, Vector2 size)
      {
        return pivot.TransformToUpperLeft(size).Scale(-1f, -1f, 1f);
      }

      public static Vector3 TransformToUpperLeft(this dfPivotPoint pivot, Vector2 size)
      {
        switch (pivot)
        {
          case dfPivotPoint.TopLeft:
            return (Vector3) new Vector2(0.0f, 0.0f);
          case dfPivotPoint.TopCenter:
            return (Vector3) new Vector2((float) (0.5 * -(double) size.x), 0.0f);
          case dfPivotPoint.TopRight:
            return (Vector3) new Vector2(-size.x, 0.0f);
          case dfPivotPoint.MiddleLeft:
            return (Vector3) new Vector2(0.0f, 0.5f * size.y);
          case dfPivotPoint.MiddleCenter:
            return (Vector3) new Vector2((float) (0.5 * -(double) size.x), 0.5f * size.y);
          case dfPivotPoint.MiddleRight:
            return (Vector3) new Vector2(-size.x, 0.5f * size.y);
          case dfPivotPoint.BottomLeft:
            return (Vector3) new Vector2(0.0f, size.y);
          case dfPivotPoint.BottomCenter:
            return (Vector3) new Vector2((float) (0.5 * -(double) size.x), size.y);
          case dfPivotPoint.BottomRight:
            return (Vector3) new Vector2(-size.x, size.y);
          default:
            throw new Exception($"Unhandled {pivot.GetType().Name} value: {(object) pivot}");
        }
      }
    }

}
