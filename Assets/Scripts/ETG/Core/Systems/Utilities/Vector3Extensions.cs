...
#nullable disable
  ...
  public static class Vector3Extensions
  {
    ...
    public static Vector3 RotateBy(this Vector3 vector, Quaternion rotation) => rotation * vector;
    ...
    public static IntVector2 IntXY(this Vector3 vector, VectorConversions convert = VectorConversions.Round)
    {
      switch (convert)
      {
        ...
        default:
          BraveUtility.Log($"Called IntXY() with an unknown conversion type ({convert})", Color.red, BraveUtility.LogVerbosity.IMPORTANT);
    ...
    {
      return !GameManager.Instance.Dungeon.data.CheckInBoundsAndValid(vector.IntXY(VectorConversions.Floor)) ? (CellData) null : GameManager.Instance.Dungeon.data[vector.IntXY(VectorConversions.Floor)];
    }
