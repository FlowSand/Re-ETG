...
// Type: dfJapaneseMarkupTokenizer
...
namespace ETG.Core.Systems.Utilities
{
    public class dfJapaneseMarkupTokenizer : IDisposable, IPoolable
    {
      ...
      {
        using (dfJapaneseMarkupTokenizer japaneseMarkupTokenizer = dfJapaneseMarkupTokenizer.pool.Count <= 0 ? new dfJapaneseMarkupTokenizer() : dfJapaneseMarkupTokenizer.pool.Pop())
          return japaneseMarkupTokenizer.tokenize(source);
      }
      ...
      private int estimateTokenCount(string source) => string.IsNullOrEmpty(source) ? 0 : source.Length;
      ...
      {
        for (int index1 = 0; index1 < dfJapaneseMarkupTokenizer.validTags.Count; ++index1)
        {
          string validTag = dfJapaneseMarkupTokenizer.validTags[index1];
          bool flag = true;
