...
// Type: InControl.TwoAxisInputControl
...
namespace InControl
{
  public class TwoAxisInputControl : IInputControl
  {
    ...
    internal void UpdateWithAxes(float x, float y, ulong updateTick, float deltaTime)
    {
      ...
      this.Left.CommitWithValue(Mathf.Max(0.0f, -this.X), updateTick, deltaTime);
      this.Right.CommitWithValue(Mathf.Max(0.0f, this.X), updateTick, deltaTime);
      if (InputManager.InvertYAxis)
      {
        this.Up.CommitWithValue(Mathf.Max(0.0f, -this.Y), updateTick, deltaTime);
        this.Down.CommitWithValue(Mathf.Max(0.0f, this.Y), updateTick, deltaTime);
      }
      else
      {
        this.Up.CommitWithValue(Mathf.Max(0.0f, this.Y), updateTick, deltaTime);
      ...
      else if (this.thisValue != this.lastValue)
      {
    ...
    public bool State => this.thisState;

    public bool LastState => this.lastState;

    public Vector2 Value => this.thisValue;

    public Vector2 LastValue => this.lastValue;

    public Vector2 Vector => this.thisValue;
    ...
    public float Angle => Utility.VectorToAngle(this.thisValue);
    ...
    {
      return (Vector3) instance.thisValue;
    }
  }
}
