...
// Type: FullInspector.Internal.fiRectUtility
...
public static class fiRectUtility
{
  ...
  public static void SplitLeftHorizontalExact(
    Rect rect,
    float leftWidth,
    float margin,
    out Rect left,
  ...
  public static void SplitRightHorizontalExact(
    Rect rect,
    float rightWidth,
    float margin,
    out Rect left,
  ...
  public static void SplitHorizontalPercentage(
    Rect rect,
    float percentage,
    float margin,
    out Rect left,
  ...
  public static void SplitHorizontalMiddleExact(
    Rect rect,
    float middleWidth,
    float margin,
    out Rect left,
  ...
  {
    left = new Rect(rect);
    left.width = (float) (((double) rect.width - 2.0 * (double) margin - (double) middleWidth) / 2.0);
    middle = new Rect(rect);
    middle.x += left.width + margin;
    middle.width = middleWidth;
    right = new Rect(rect);
    right.x += left.width + margin + middleWidth + margin;
    right.width = (float) (((double) rect.width - 2.0 * (double) margin - (double) middleWidth) / 2.0);
  }

  public static void SplitHorizontalFlexibleMiddle(
    Rect rect,
    float leftWidth,
    float rightWidth,
    out Rect left,
  ...
  public static void SplitVerticalPercentage(
    Rect rect,
    float percentage,
    float margin,
    out Rect top,
