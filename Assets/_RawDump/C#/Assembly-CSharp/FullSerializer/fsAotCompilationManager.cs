...
// Type: FullSerializer.fsAotCompilationManager
...
public class fsAotCompilationManager
{
    ...
    bool isConstructorPublic)
  {
    ...
    foreach (fsMetaProperty member in members)
      stringBuilder.AppendLine($"            result += SerializeMember(serialized, \"{member.JsonName}\", model.{member.MemberName});");
    stringBuilder.AppendLine();
    ...
    for (int index = 0; index < members.Length; ++index)
    {
      ...
      stringBuilder.AppendLine($"            result += DeserializeMember(data, \"{member.JsonName}\", out t{(object) index});");
