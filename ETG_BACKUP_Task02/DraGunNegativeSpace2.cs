...
public class DraGunNegativeSpace2 : ScriptLite
{
  ...
  public override void Start()
  {
    ...
    this.m_platformCenters.Add(new Vector2(Random.Range(-17f, 17f) / 2f, 0.0f));
    for (int index1 = 1; index1 < 10; ++index1)
    {
      Vector2 platformCenter = this.m_platformCenters[this.m_platformCenters.Count - 1];
      DraGunNegativeSpace2.s_validPlatformIndices.Clear();
      for (int index2 = 0; index2 < DraGunNegativeSpace2.PlatformAngles.Length; ++index2)
      {
        Vector2 a = platformCenter + BraveMathCollege.DegreesToVector(DraGunNegativeSpace2.PlatformAngles[index2], 2f * this.ActivePlatformRadius + DraGunNegativeSpace2.PlatformDistances[index2]);
        if ((double) a.x > -17.0 && (double) a.x < 17.0 && (double) Vector2.Distance(a, this.m_platformCenters[this.m_platformCenters.Count - 1]) > (double) num && (index1 < 2 || (double) Vector2.Distance(a, this.m_platformCenters[this.m_platformCenters.Count - 2]) > (double) num) && (index1 < 3 || (double) Vector2.Distance(a, this.m_platformCenters[this.m_platformCenters.Count - 3]) > (double) num))
          DraGunNegativeSpace2.s_validPlatformIndices.Add(index2);
      }
      if (DraGunNegativeSpace2.s_validPlatformIndices.Count == 0)
        DraGunNegativeSpace2.s_validPlatformIndices.Add(2);
      int index3 = BraveUtility.RandomElement<int>(DraGunNegativeSpace2.s_validPlatformIndices);
      this.m_platformCenters.Add(platformCenter + BraveMathCollege.DegreesToVector(DraGunNegativeSpace2.PlatformAngles[index3], 2f * this.ActivePlatformRadius + DraGunNegativeSpace2.PlatformDistances[index3]));
      if (index1 % 2 == 1)
      {
        DraGunNegativeSpace2.s_validPlatformIndices.Remove(index3);
        for (int index4 = DraGunNegativeSpace2.s_validPlatformIndices.Count - 1; index4 >= 0; --index4)
        {
          int validPlatformIndex = DraGunNegativeSpace2.s_validPlatformIndices[index4];
          Vector2 a = platformCenter + BraveMathCollege.DegreesToVector(DraGunNegativeSpace2.PlatformAngles[validPlatformIndex], 2f * this.ActivePlatformRadius + DraGunNegativeSpace2.PlatformDistances[validPlatformIndex]);
          if ((double) Vector2.Distance(a, this.m_platformCenters[this.m_platformCenters.Count - 1]) < (double) num || (double) Vector2.Distance(a, this.m_platformCenters[this.m_platformCenters.Count - 2]) < (double) num)
            DraGunNegativeSpace2.s_validPlatformIndices.RemoveAt(index4);
        }
        if (DraGunNegativeSpace2.s_validPlatformIndices.Count > 0)
        {
          int index5 = BraveUtility.RandomElement<int>(DraGunNegativeSpace2.s_validPlatformIndices);
          this.m_platformCenters.Add(platformCenter + BraveMathCollege.DegreesToVector(DraGunNegativeSpace2.PlatformAngles[index5], 2f * this.ActivePlatformRadius + DraGunNegativeSpace2.PlatformDistances[index5]));
        }
      }
    }
  ...
  public override int Update(ref int state)
  {
    ...
    if ((double) this.m_rowHeight < (double) this.m_lastCenterHeight)
    {
      for (int i = 0; i < 19; ++i)
      {
        ...
        this.Fire(new Offset(x, 18f, transform: string.Empty), new Brave.BulletScript.Direction(-90f), new Brave.BulletScript.Speed(6f), (Bullet) new DraGunNegativeSpace2.WiggleBullet(suppressOffset));
      }
      this.m_rowHeight += this.m_verticalGap;
      for (int index = 0; index < this.m_platformCenters.Count; ++index)
      {
        if (!this.m_centerBullets[index] && (double) this.m_platformCenters[index].y < (double) this.m_rowHeight - 2.0)
        {
          if (BraveUtility.RandomBool())
            this.Fire(new Offset(this.m_platformCenters[index].x, 20.5f - this.m_rowHeight + this.m_platformCenters[index].y, transform: string.Empty), new Brave.BulletScript.Direction(-90f), new Brave.BulletScript.Speed(6f), (Bullet) new DraGunNegativeSpace2.WiggleBullet(true));
